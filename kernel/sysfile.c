...
//find out whether a page is previously lazy-allocated for a vma
...
int vmatrylazytouch(uint64 va){
  ...
  //set appropriate prems, then map it.
  int perm = PTE_U;
  if(v->prot & PROT_READ){
    perm |= PTE_R;
  }
...
sys_munmap(void){
  ...
  //printf("munmap: addr=%p sz=%p vma: start=%p sz=%p\n", addr, sz, v->vastart, v->sz);
  if(v == 0){
    printf("no vma");
    return -1;
  }
